        ...
        <div class="card">
            <div class="card-body">
                <table id="TabelData1" class="table table-bordered table-sm">
                    ...
                    <tbody>
                        @php $no = 1; @endphp
                        @foreach($kib_tanah as $baris)
                        <tr>
                            <td class="text-center">{{ $no++ }}</td>
                            <td class="text-center">{{ $baris->kode_barang }}</td>
                            <td>{{ $baris->reg_barang }}</td>
                            <td>{{ $baris->jenis_barang }}</td>
                            <td>{{ $baris->luas }}</td>
                            <td>{{ $baris->tahun_beli }}</td>
                            <td>{{ $baris->alamat }}</td>
                            <td class="text-center">
                                <a href="/surma/buka/{{ $baris->kode_barang}}" target="_blank" class="btn btn-xs btn-purple"><i class="fa fa-folder-open mr-1"></i></a>
                            </td>
                            <td class="text-center">
                                <a href="#edit{{$baris->kode_barang}}" data-bs-toggle="modal" class="btn btn-xs btn-green"><i class="fa fa-edit mr-1"></i></a>

                                <a href="#delete{{$baris->kode_barang}}" data-bs-toggle="modal" class="btn btn-xs btn-danger"><i class="fa fa-trash-alt mr-1"></i></a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
...
<div class="modal fade" id="importexcel">
    <div class="modal-dialog modal-lg">
        <div class=" modal-content">
            <form action="/tanah/import" method="POST" enctype="multipart/form-data">
                <?= csrf_field() ?>
...
<!-- ########## Modal Edit ########## -->
@foreach($kib_tanah as $baris)
<div class="modal fade" id="edit{{$baris->kode_barang}}">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">

            <form action="/surma/{{$baris->kode_barang}}/update/" method="POST" id="frmarsip" enctype="multipart/form-data">
                @csrf
                        ...
                        <div class="col-xl-12">
                            <div class="border p-4 rounded">
                                ...
                                <div class="row mb-1">
                                    <label class="col-sm-2 col-form-label"><b>No. Agenda</b></label>
                                    <div class="col-sm-4">
                                        <input class="form-control form-control-sm bg-secondary text-white" type="text" id="kode_barang" name="kode_barang" value="{{$baris->kode_barang}}" readonly="True">
                                    </div>
                                </div>
                                <div class="row mb-1">
                                    <label class="col-sm-2 col-form-label"><b>Tgl. Diterima</b></label>
                                    <div class="col-sm-4">
                                        <div class="input-group input-group-sm">
                                            <input type="text" class="form-control form-control-sm date-picker" name="tgl_diterima" id="tgl_diterima" value="{{ date('d-m-Y', strtotime($baris->tgl_sertifikat)) }}">
                                            <span class="input-group-text"><i class="fa fa-calendar"></i></span>
                                        </div>
                                    </div>
                                </div>
                                <div class="row mb-1">
                                    <label class="col-sm-2 col-form-label"><b>No. Surat</b></label>
                                    <div class="col-sm-10">
                                        <input class="form-control form-control-sm" type="text" id="no_surat" name="no_surat" value="{{$baris->no_sertifikat}}">
                                    </div>
                                </div>
                                <div class="row mb-1">
                                    <label class="col-sm-2 col-form-label"><b>Tgl. Surat</b></label>
                                    <div class="col-sm-4">
                                        <div class="input-group input-group-sm">
                                            <input type="text" class="form-control form-control-sm date-picker" name="tgl_surat" id="tgl_surat" value="{{ date('d-m-Y', strtotime($baris->tgl_sertifikat)) }}">
                                            <span class="input-group-text"><i class="fa fa-calendar"></i></span>
                                        </div>
                                    </div>
                                </div>
...
<!-- ########## Modal Hapus ########## -->
@foreach($kib_tanah as $baris)
<div class="modal fade" id="delete{{$baris->kode_barang}}">
    <div class="modal-dialog modal-xs">
        <div class=" modal-content">
            ...
            <div class="modal-footer">
                <a href="/surma/{{$baris->kode_barang}}/hapus/" class="btn btn-sm btn-primary"><i class="fa fa-trash-alt mr-1"></i> Hapus Data</a>
